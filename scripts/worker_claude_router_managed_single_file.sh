...
STATUS: <SUCCESS|NEEDS_REPLAN|OVERFLOW_DETECTED|ROUTER_ERROR|CHILD_LIMIT_REACHED|CHILD_TIMEOUT|FAILED>
FILES: ${TARGET_FILE}
...
elif [[ "$PARSED_STATUS_LINE" = "ROUTER_ERROR" ]] || grep -Eiq 'connection refused|connection reset|upstream unavailable|router error|ECONNRESET|empty response from router' "$EXEC_LOG_FILE"; then
  STATUS="ROUTER_ERROR"
elif [[ ! -s "$RAW_FILE" ]] || [[ -z "$PARSED_STATUS_LINE" ]] || [[ ! "$PARSED_STATUS_LINE" =~ ^(SUCCESS|NEEDS_REPLAN|OVERFLOW_DETECTED|ROUTER_ERROR|CHILD_LIMIT_REACHED|CHILD_TIMEOUT|FAILED)$ ]]; then
  STATUS="ROUTER_ERROR"
