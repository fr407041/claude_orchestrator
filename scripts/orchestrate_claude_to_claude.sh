...
source "${SCRIPT_DIR}/orchestrator_limits.sh"
...
MAX_FILES_PER_JOB="${ORCH_MAX_FILES_PER_JOB}"
MAX_JOBS="${ORCH_MAX_JOBS}"
INVENTORY_LIMIT="${ORCH_INVENTORY_LIMIT}"
EXECUTE_WORKERS="${ORCH_EXECUTE_WORKERS}"
WORKER_MODE="${ORCH_WORKER_MODE}"
MAX_CHILD_INVOCATIONS="${ORCH_MAX_CHILD_INVOCATIONS}"
MAX_RETRIES_PER_JOB="${ORCH_MAX_RETRIES_PER_JOB}"
MAX_FAIL_REPLANS_PER_JOB="${ORCH_MAX_FAIL_REPLANS_PER_JOB}"
MAX_ROUTER_RECOVERY_REPLANS="${ORCH_MAX_ROUTER_RECOVERY_REPLANS}"
MAX_FILE_LINES="${ORCH_MAX_FILE_LINES}"
RUN_ROOT="${ORCH_RUN_ROOT:-${REPO_ROOT}/orchestrator-claude}"
DEFAULT_RUN_ID="$(python3 - <<'PY'
from datetime import datetime
import os
print(f"run-{datetime.now().strftime('%Y%m%d-%H%M%S-%f')}-{os.getpid()}")
PY
)"
RUN_ID="${ORCH_RUN_ID:-${DEFAULT_RUN_ID}}"
...
elif "ROUTER_ERROR" in combined or "connection refused" in combined.lower() or "connection reset" in combined.lower() or "upstream unavailable" in combined.lower() or "empty response from router" in combined.lower():
    status = "ROUTER_ERROR"
...
run_single_file_overflow_retry() {
  local source_job_file="${1:?source_job_file required}"
  local status_prefix_base="${2:?status_prefix_base required}"
  local current_source_job="$source_job_file"
  local attempt=1

  while (( OVERFLOW_RETRIES < MAX_RETRIES_PER_JOB )); do
    local suffix
    local replan_job
    local replan_file_count
    local forced_mode
    local replan_worker_script
    local replan_status_file
    local replan_status
    local replan_changed_count
    local replan_verification_note

    OVERFLOW_RETRIES=$((OVERFLOW_RETRIES + 1))
    suffix="${status_prefix_base}-$(printf '%02d' "$attempt")"
    create_replan_job \
      "$current_source_job" \
      "$JOBS_DIR" \
      "${suffix}" \
      "REPLAN_HINT: the previous child Claude attempt overflowed even on a narrow scope. Retry on the same file, keep the response extremely short, and return only the minimum valid final content."

    replan_job="${JOBS_DIR}/$(basename "${current_source_job%.json}")-${suffix}.json"
    if [[ ! -f "$replan_job" || "$CHILD_INVOCATION_LIMIT_HIT" -ne 0 ]]; then
    ...
    if [[ "$replan_status" = "CHILD_LIMIT_REACHED" ]]; then
      ...
      REPLAN_LOOP_GUARD_HIT=1
      return 0
    elif [[ "$replan_status" = "ROUTER_ERROR" ]]; then
      WORKERS_ROUTER_ERRORS=$((WORKERS_ROUTER_ERRORS + 1))
      WORKERS_FAILED=$((WORKERS_FAILED + 1))
      REPLAN_LOOP_GUARD_HIT=1
      return 0
      ...
      if (( OVERFLOW_RETRIES >= MAX_RETRIES_PER_JOB )); then
        WORKERS_FAILED=$((WORKERS_FAILED + 1))
        REPLAN_LOOP_GUARD_HIT=1
        return 0
      fi
      current_source_job="$replan_job"
      attempt=$((attempt + 1))
      continue
...
run_router_recovery_retry() {
  local source_job_file="${1:?source_job_file required}"
  local current_source_job="$source_job_file"
  local attempt=1

  while (( attempt <= MAX_ROUTER_RECOVERY_REPLANS )); do
    local suffix
    local replan_job
    local replan_file_count
    local forced_mode
    ...
    ROUTER_REPLAN_ATTEMPTS=$((ROUTER_REPLAN_ATTEMPTS + 1))
    WORKERS_FAILED_REPLANNED=$((WORKERS_FAILED_REPLANNED + 1))
    suffix="router-retry-$(printf '%02d' "$attempt")"
    create_replan_job \
      "$current_source_job" \
      "$JOBS_DIR" \
      "$suffix" \
      "REPLAN_HINT: the previous child Claude attempt hit a router or upstream transport failure. Retry with the smallest safe scope, keep the response minimal, and avoid broad redispatch."
    replan_job="${JOBS_DIR}/$(basename "${current_source_job%.json}")-${suffix}.json"
    ...
    if [[ "$replan_status" = "CHILD_LIMIT_REACHED" ]]; then
      ...
      WORKERS_ROUTER_ERRORS=$((WORKERS_ROUTER_ERRORS + 1))
      if (( attempt >= MAX_ROUTER_RECOVERY_REPLANS )); then
        WORKERS_FAILED=$((WORKERS_FAILED + 1))
        REPLAN_LOOP_GUARD_HIT=1
        return 0
      fi
      ...
      WORKERS_OVERFLOWED=$((WORKERS_OVERFLOWED + 1))
      if [[ "$replan_file_count" -eq 1 ]]; then
        run_single_file_overflow_retry "$replan_job" "overflow-retry"
      else
        REPLAN_LOOP_GUARD_HIT=1
      fi
      return 0
...
def infer_test_command(files, scope_path: str) -> str:
    ...
    if files and any(path.endswith(".py") for path in files):
        return "python3 -m pytest -q"
    return ""
...
if [[ "$EXECUTE_WORKERS" = "1" ]]; then
  for job_file in "$JOBS_DIR"/job-*.json; do
    ...
    if [[ "$MAX_FILE_LINES" -gt 0 ]]; then
      large_guard_result="$(python3 - "$job_file" "$MAX_FILE_LINES" <<'PY'
import json
import sys
from pathlib import Path
...
oversized = []
scope_path = Path(job.get("scope_path") or ".")
for rel in job.get("files", []):
    ...
    try:
        ...
        continue
    if lines > limit:
        oversized.append({"path": rel, "lines": lines})
print(json.dumps({"triggered": bool(oversized), "oversized": oversized}))
PY
)"
      large_guard_triggered="$(python3 - "$large_guard_result" <<'PY'
import json
import sys
print("1" if json.loads(sys.argv[1]).get("triggered") else "0")
PY
)"
      if [[ "$large_guard_triggered" = "1" ]]; then
        LARGE_FILE_GUARD_TRIGGERED=$((LARGE_FILE_GUARD_TRIGGERED + 1))
        python3 - "$job_file" "$status_file" "$large_guard_result" <<'PY'
import json
import sys
from pathlib import Path
...
job = json.loads(Path(sys.argv[1]).read_text(encoding="utf-8"))
status_path = Path(sys.argv[2])
guard = json.loads(sys.argv[3])
details = ", ".join(f"{item['path']}({item['lines']} lines)" for item in guard.get("oversized", []))
payload = {
    "id": job.get("id", status_path.stem),
    "status": "LARGE_FILE_GUARD",
    "scope_path": job.get("scope_path", ""),
    "require_change": job.get("require_change", False),
    "files": job.get("files", []),
    "actual_changed_files": [],
    "actual_changed_count": 0,
    "verification_note": f"large file guard blocked direct edit: {details}",
    "raw_file": "",
    "exec_log_file": "",
    "success_check": job.get("success_check", ""),
    "test_command": job.get("test_command", ""),
...
PY
    ...
    if [[ "$job_status" = "CHILD_LIMIT_REACHED" ]]; then
      WORKERS_CHILD_LIMIT_BLOCKED=$((WORKERS_CHILD_LIMIT_BLOCKED + 1))
    elif [[ "$job_status" = "ROUTER_ERROR" ]]; then
      WORKERS_ROUTER_ERRORS=$((WORKERS_ROUTER_ERRORS + 1))
      if [[ "$require_change" = "true" && "$MAX_ROUTER_RECOVERY_REPLANS" -gt 0 ]]; then
        run_router_recovery_retry "$job_file"
      else
        WORKERS_FAILED=$((WORKERS_FAILED + 1))
        REPLAN_LOOP_GUARD_HIT=1
      fi
    elif [[ "$job_status" = "LARGE_FILE_GUARD" ]]; then
      :
...
PY
        for retry_job in "$JOBS_DIR"/"$(basename "${job_file%.json}")"-retry-*.json; do
          ...
          if [[ "$retry_status" = "CHILD_LIMIT_REACHED" ]]; then
            WORKERS_CHILD_LIMIT_BLOCKED=$((WORKERS_CHILD_LIMIT_BLOCKED + 1))
          elif [[ "$retry_status" = "ROUTER_ERROR" ]]; then
            WORKERS_ROUTER_ERRORS=$((WORKERS_ROUTER_ERRORS + 1))
            WORKERS_FAILED=$((WORKERS_FAILED + 1))
            REPLAN_LOOP_GUARD_HIT=1
          elif [[ "$retry_status" = "OVERFLOW_DETECTED" ]]; then
            WORKERS_OVERFLOWED=$((WORKERS_OVERFLOWED + 1))
            if [[ "$retry_file_count" -eq 1 && "$OVERFLOW_RETRIES" -lt "$MAX_RETRIES_PER_JOB" ]]; then
              run_single_file_overflow_retry "$retry_job" "overflow-retry"
            else
              WORKERS_FAILED=$((WORKERS_FAILED + 1))
            fi
      ...
      elif [[ "$file_count" -eq 1 && "$OVERFLOW_RETRIES" -lt "$MAX_RETRIES_PER_JOB" ]]; then
        run_single_file_overflow_retry "$job_file" "overflow-retry"
      else
        WORKERS_FAILED=$((WORKERS_FAILED + 1))
      fi
          ...
          if (( WORKERS_RUN >= MAX_CHILD_INVOCATIONS )); then
            ...
            if [[ "$replan_status" = "CHILD_LIMIT_REACHED" ]]; then
              ...
              if [[ "$job_status" = "FAILED" ]]; then
                SAME_FAILURE_AGAIN=$((SAME_FAILURE_AGAIN + 1))
              fi
              WORKERS_FAILED=$((WORKERS_FAILED + 1))
              REPLAN_LOOP_GUARD_HIT=1
            fi
        ...
        if [[ -f "$replan_job" && "$CHILD_INVOCATION_LIMIT_HIT" -eq 0 ]]; then
          if (( WORKERS_RUN >= MAX_CHILD_INVOCATIONS )); then
            ...
            if [[ "$replan_status" = "CHILD_LIMIT_REACHED" ]]; then
              ...
              if [[ "$job_status" = "FAILED" || "$job_status" = "CHILD_TIMEOUT" ]]; then
                SAME_FAILURE_AGAIN=$((SAME_FAILURE_AGAIN + 1))
              fi
              WORKERS_FAILED=$((WORKERS_FAILED + 1))
              REPLAN_LOOP_GUARD_HIT=1
...
jq -n \
  ...
  --argjson workers_router_errors "$WORKERS_ROUTER_ERRORS" \
  --argjson large_file_guard_triggered "$LARGE_FILE_GUARD_TRIGGERED" \
  ...
  '{
    ...
    metrics: {
      ...
      workers_router_errors: $workers_router_errors,
      large_file_guard_triggered: $large_file_guard_triggered,
