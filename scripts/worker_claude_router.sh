...
elif grep -Eiq '^STATUS:\s*ROUTER_ERROR' "$RAW_FILE" || grep -Eiq 'connection refused|connection reset|upstream unavailable|router error|ECONNRESET|empty response from router' "$RAW_FILE"; then
  STATUS="ROUTER_ERROR"
elif [[ ! -s "$RAW_FILE" ]] || ! grep -Eiq '^STATUS:\s*(SUCCESS|NEEDS_REPLAN|OVERFLOW_DETECTED|ROUTER_ERROR|CHILD_LIMIT_REACHED|CHILD_TIMEOUT|FAILED)\b' "$RAW_FILE"; then
  STATUS="ROUTER_ERROR"
